...
namespace ProjectR.UI
{
    ...
    public class PopupHotkeyController : SWSingleton<PopupHotkeyController>
    {
        ...
        private void HandlePopupHidden(SWPopupBase popup)
        {
            ...
            {
                if (ReferenceEquals(entries[i].OpenedPopup, popup) == false) continue;

                entries[i].OpenedPopup = null;
                return;
            }
        }
        ...
        private void Update()
        {
            ...
            for (int i = 0; i < entries.Count; i++)
            {
                if (Keyboard.current[entries[i].Key].wasPressedThisFrame == false) continue;
